...
class NumbersGameActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            override fun onResults(results: Bundle?) {
                ...
                if (matches != null && matches.isNotEmpty()) {
                    ...
                    if (spokenText == currentAnswer) {
                        ...
                        wrongSound.start()
                        Toast.makeText(this@NumbersGameActivity, "Wrong answer. Try again!", Toast.LENGTH_SHORT).show()
                        finish()
                    }
                }
            }
        ...
        // Generate the first random two-digit number
        generateNewNumber()
    }

    // Function to generate a new random two-digit number
    private fun generateNewNumber() {
        currentAnswer = random.nextInt(90) + 10 // This ensures the number will be between 10 and 99.
        findViewById<TextView>(R.id.numberTextView).text = currentAnswer.toString()
    }
    ...
    private fun incrementScore() {
        ...
        if (score > highScore) {
            // If the score is higher than the current high score, update high score and save to shared preferences
            highScore = score
            getSharedPreferences("AlphaMath", Context.MODE_PRIVATE).edit()
                .putInt("numbers_high_score", highScore)
                .apply()
            findViewById<TextView>(R.id.highScoreTextView).text = "High Score: $highScore"
        }
        // Generate a new random two-digit number
        generateNewNumber()
    }
}
